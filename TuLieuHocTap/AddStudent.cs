...
namespace TuLieuHocTap
{
	public partial class AddStudent : Form
	{
        ...
        public AddStudent()
		{
            ...
            {
                MessageBox.Show("Ấn để tiếp tục :))))");
            }
        ...
        private void addtodatabase(byte[] imgface) //adds to database
        {
            ...
            {
                if (textBox1.Text == String.Empty || textBox2.Text == String.Empty || comboBox1.Text == String.Empty || comboBox2.Text == String.Empty)
                {
                ...
                else
                {
                    ...
                    {
                        Connection.Open();
                        SqlCommand cmd = new SqlCommand(@"INSERT INTO student ([name], [studentid],[imgface],[makhoa],[malop]) VALUES (@studentname, @idstudent,@imgface,@makhoa,@malop);", Connection);
                        cmd.Parameters.AddWithValue("@studentname", textBox1.Text);
                        cmd.Parameters.AddWithValue("@idstudent", textBox2.Text);
                        cmd.Parameters.AddWithValue("@imgface", imgface);
                        cmd.Parameters.AddWithValue("@makhoa", comboBox1.Text);
                        cmd.Parameters.AddWithValue("@malop", comboBox2.Text);
                        int i = cmd.ExecuteNonQuery();
                        Connection.Close();

                        if (i == 1)
                        {
                            MessageBox.Show("Học sinh đã được lưu vào danh sách");
                            getPrimaryKey();
                            textBox1.Clear();
                            textBox2.Clear();
